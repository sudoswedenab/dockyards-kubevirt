...
func TestDockyardsNodePoolReconciler_ReconcileTalosControlPlane(t *testing.T) {
	...
	t.Run("test controlplane ntp servers", func(t *testing.T) {
		...
		_, err = reconcilerWithNTP.reconcileTalosControlPlane(ctx, &nodePool, &owner)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("test controlplane ptp devices", func(t *testing.T) {
		...
		_, err = reconcilerWithPTP.reconcileTalosControlPlane(ctx, &nodePool, &owner)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("test controlplane ntp servers and ptp devices", func(t *testing.T) {
		...
		_, err = reconcilerWithTimeSync.reconcileTalosControlPlane(ctx, &nodePool, &owner)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("test controlplane static routes", func(t *testing.T) {
		reconcilerWithRoutes := reconciler
		reconcilerWithRoutes.DockyardsConfig = dyconfig.NewFakeConfigManager(map[string]string{
			string(KeyStaticRoutes): `
eth0:
...
`,
		...
		_, err = reconcilerWithRoutes.reconcileTalosControlPlane(ctx, &nodePool, &owner)
		if err != nil {
			t.Fatal(err)
		}
		...
		metric := uint32(100)
		routesPatch, err := yaml.Marshal(talospatchv1.LinkConfig{
			Meta: talospatchv1.Meta{APIVersion: talospatchv1.LinkConfigAPIVersion, Kind: talospatchv1.LinkConfigKind},
			Name: "eth0",
			Routes: []talospatchv1.RouteConfig{
				{Destination: "10.0.0.0/8", Gateway: "10.0.0.1", Metric: &metric},
	...
	t.Run("test controlplane dhcpv4 interface name", func(t *testing.T) {
		...
		_, err = reconcilerWithDHCP.reconcileTalosControlPlane(ctx, &nodePool, &owner)
		if err != nil {
			t.Fatal(err)
		}
		...
		dhcpPatch, err := yaml.Marshal(talospatchv1.DHCPv4Config{
...
func TestDockyardsNodePoolReconciler_ReconcileTalosConfigTemplate(t *testing.T) {
	...
	t.Run("test config template static routes", func(t *testing.T) {
		owner := dockyardsv1.Cluster{
			ObjectMeta: metav1.ObjectMeta{
				GenerateName: "test",
				Namespace:    namespace.Name,
...
func TestDockyardsNodePoolReconciler_ReconcileMachineTemplateMultus(t *testing.T) {
	if os.Getenv("KUBEBUILDER_ASSETS") == "" {
		t.Skip("no kubebuilder assets configured")
	}

	env := envtest.Environment{
		CRDs: append(mockcrds.CRDs, mockcrds.MultusCRDs...),
	}
								...
								Template: &kubevirtv1.VirtualMachineInstanceTemplateSpec{
									Spec: kubevirtv1.VirtualMachineInstanceSpec{
										...
										Networks: []kubevirtv1.Network{
											{
												Name:          "default",
												NetworkSource: kubevirtv1.DefaultPodNetwork().NetworkSource,
											},
										},
